...
export default function EditSupportGroupPage() {
  ...
  // Load categories + group data in parallel
  useEffect(() => {
    Promise.all([
      api.get<{ success: boolean; data: Category[] }>("/group-categories"),
      api.get<{ success: boolean; data: GroupDetail }>(`/groups/${groupId}`),
    ])
      .then(([catsRes, groupRes]) => {
        ...
        setExistingCover(getImageUrl(g.cover_image));
        setExistingImage(getImageUrl(g.group_image));

        const selectedSubs = (g.subcategories ?? []).map(s => s.id);
        ...
        setForm({
          ...
          group_privacy: g.group_privacy || "public",
        ...
        if (g.category_id) {
          setLoadingSubs(true);
          api.get<{ success: boolean; data: SubCategory[] }>("/group-subcategories", { params: { category_id: g.category_id } })
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-24 pb-16">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <ChevronRight className="w-3.5 h-3.5" />
          <span className="text-gray-700 font-medium">Edit Group</span>
        </div>

        <h1 className="text-2xl font-bold text-gray-900 mb-6">Edit Support Group</h1>
