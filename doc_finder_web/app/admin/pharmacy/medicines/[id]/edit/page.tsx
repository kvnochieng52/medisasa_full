...
export default function EditMedicinePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setForm({
        name: m.name || "",
        medicine_number: m.medicine_number || "",
        ...
        strength: m.strength || "",
        form: m.form || "",
        quantity_available: String(m.quantity_available ?? 0),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post(`/medicines/${id}`, fd, { headers: { "Content-Type": "multipart/form-data" } });
      ...
      else toast.error(e?.response?.data?.message ?? "Failed to update medicine");
    } finally {
      setSaving(false);
    }
  };
