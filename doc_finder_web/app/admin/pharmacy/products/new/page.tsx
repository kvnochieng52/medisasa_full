...
import { ChevronRight, Loader2, Upload, ShoppingBag, ChevronDown, ShieldAlert } from "lucide-react";
...
export default function NewMedicalProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.stock_quantity) { toast.error("Stock quantity is required"); return; }
    ...
    try {
      ...
      await api.post("/medical-products", fd, { headers: { "Content-Type": "multipart/form-data" } });
      ...
      else toast.error(e?.response?.data?.message ?? "Failed to save product");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-24 pb-16">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Add Medical Product</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-center">
            <div onClick={() => imageRef.current?.click()}
              ...
              {imagePreview
                ...
                : <div className="text-center"><Upload className="w-6 h-6 text-purple-400 mx-auto" /><p className="text-xs text-purple-400 mt-1">Add Photo</p></div>}
              {imagePreview && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Product Name" required>
                <input type="text" value={form.name} onChange={e => set("name", e.target.value)}
                  placeholder="e.g. Aspirin 100mg" className={inputCls} />
              </Field>
              <Field label="Batch Number" required>
                <input type="text" value={form.batch_no} onChange={e => set("batch_no", e.target.value)}
                  placeholder="e.g. BATCH-2025-001" className={inputCls} />
              </Field>
            </div>
            <FacilityPicker
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Supplier">
                <input type="text" value={form.supplier} onChange={e => set("supplier", e.target.value)}
                  placeholder="e.g. MedSupply Ltd" className={inputCls} />
              </Field>
              <Field label="Barcode">
          ...
          <Section title="Product Details">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Field label="Strength">
                <input type="text" value={form.strength} onChange={e => set("strength", e.target.value)}
                  placeholder="e.g. 100mg" className={inputCls} />
              </Field>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Weight (grams)">
                <input type="number" min="0" step="0.001" value={form.weight} onChange={e => set("weight", e.target.value)}
                  placeholder="0.000" className={inputCls} />
              </Field>
              <Field label="Storage Conditions">
                <input type="text" value={form.storage_conditions} onChange={e => set("storage_conditions", e.target.value)}
                  placeholder="e.g. Store below 25°C" className={inputCls} />
              </Field>
            </div>
            <Field label="Usage Instructions">
              <textarea rows={2} value={form.usage_instructions} onChange={e => set("usage_instructions", e.target.value)}
                placeholder="How to use this product…" className={`${inputCls} resize-none`} />
            </Field>
          </Section>
          ...
          <Section title="Clinical Information">
            <Field label="Conditions Treated" hint="Comma-separated">
              <input type="text" value={form.conditions} onChange={e => set("conditions", e.target.value)}
                placeholder="e.g. headache, fever" className={inputCls} />
            </Field>
            <Field label="Side Effects" hint="Comma-separated">
              <input type="text" value={form.side_effects} onChange={e => set("side_effects", e.target.value)}
                placeholder="e.g. nausea, dizziness" className={inputCls} />
            </Field>
            <Field label="Ingredients" hint="Comma-separated">
              <input type="text" value={form.ingredients} onChange={e => set("ingredients", e.target.value)}
                placeholder="e.g. aspirin, starch" className={inputCls} />
            </Field>
            <div className="flex flex-col sm:flex-row gap-3">
              <label className="flex items-center gap-3 flex-1 p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50">
                ...
                <div><p className="text-sm font-semibold text-gray-800">Requires Prescription</p></div>
              </label>
              <label className="flex items-center gap-3 flex-1 p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50">
                ...
                <div><p className="text-sm font-semibold text-gray-800">Available for Purchase</p></div>
              </label>
            </div>
          </Section>
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving…</> : <><ShoppingBag className="w-4 h-4" /> Add Product</>}
            </button>
          </div>
        </form>
      </div>
