...
import {
  Scan, Calendar, User as UserIcon, ArrowLeft, Download, Mail, Loader2, Stethoscope,
} from "lucide-react";
...
interface RadiologyItem {
  ...
  side?: string | null;
  contrast: "none" | "with" | "without" | "oral";
  urgency: "routine" | "urgent" | "stat";
  clinical_indication?: string | null;
  notes?: string | null;
}

interface RadiologyRx {
  id: number;
...
const urgencyStyle: Record<RadiologyItem["urgency"], string> = {
  routine: "bg-teal-50 text-teal-700",
  urgent: "bg-amber-50 text-amber-700",
  stat: "bg-red-50 text-red-700",
};

const contrastLabel: Record<RadiologyItem["contrast"], string> = {
  none: "No contrast",
  with: "With contrast",
...
export default function RadiologyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();
  const [rx, setRx] = useState<RadiologyRx | null>(null);
  ...
  useEffect(() => {
    ...
    api.get(`/prescriptions/radiology/${id}`)
      .then((res) => {
        const data = res.data?.data;
        setRx(data);
        setEmailTo(data?.patient_email ?? "");
      })
      .catch(() => toast.error("Failed to load radiology order."))
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/prescriptions/radiology/${id}/pdf`, {
      ...
      a.href = url;
      a.download = `${rx?.prescription_number ?? "radiology-order"}.pdf`;
  ...
  const sendEmail = async () => {
    ...
    setEmailing(true);
    try {
      await api.post(`/prescriptions/radiology/${id}/email`, { email: emailTo.trim() });
    ...
    return (
      <main className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto px-4 pt-32 pb-12 text-center">
          <p className="text-gray-500">Radiology order not found.</p>
          <Link href="/prescriptions" className="text-rose-500 hover:text-rose-600 font-semibold text-sm mt-2 inline-block">
    ...
    <main className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <h1 className="text-2xl font-bold text-white">Radiology Order</h1>
              </div>
              <p className="text-rose-50 text-sm font-mono">{rx.prescription_number}</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={downloadPdf}
                disabled={downloading}
                className="inline-flex items-center gap-2 bg-white text-rose-600 hover:bg-rose-50 text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-sm"
              >
          ...
          {showEmail && (
            <div className="mt-4 bg-white rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row items-stretch gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm outline-none focus:bg-white focus:border-rose-400 focus:ring-2 focus:ring-rose-100"
              />
              <button
                onClick={sendEmail}
                disabled={emailing || !emailTo.trim()}
                className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 disabled:bg-gray-300 text-white text-sm font-semibold transition-colors"
              >
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 pb-12 space-y-4">
        ...
        <Card title="Patient" icon={UserIcon}>
          ...
          <KV label="Sex" value={rx.patient_sex ? rx.patient_sex[0].toUpperCase() + rx.patient_sex.slice(1) : null} />
        ...
        <Card title="Studies ordered" icon={Scan}>
            ...
            {rx.items.map((it, i) => (
              <div key={it.id} className="border border-gray-100 rounded-xl p-3 bg-gray-50/50">
                ...
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600">
                  {it.modality && <span>Modality: <strong className="text-gray-800">{it.modality}</strong></span>}
                  {it.body_part && (
                    <span>Region: <strong className="text-gray-800">{it.body_part}{it.side ? ` · ${it.side}` : ""}</strong></span>
                  )}
                  <span>{contrastLabel[it.contrast]}</span>
                </div>
                {it.clinical_indication && (
                  <p className="mt-2 text-xs text-gray-700 bg-white border border-gray-100 rounded-lg px-2.5 py-1.5">
                    <em className="text-gray-500">Indication:</em> {it.clinical_indication}
                  </p>
                )}
                {it.notes && (
