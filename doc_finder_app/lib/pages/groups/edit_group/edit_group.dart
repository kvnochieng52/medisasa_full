...
class _EditGroupPageState extends State<EditGroupPage> {
  ...
  Future<void> _loadGroupData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true && responseData['group'] != null) {
          setState(() {
            _groupData = responseData['group'];
            _populateFormFields();
          });
        } else {
          _showMessage('Failed to load group data', isError: true);
        }
      } else {
        _showMessage('Group not found or access denied', isError: true);
        GoRouter.of(context).pop();
      }
    } catch (e) {
      debugPrint('Error loading group data: $e');
      _showMessage('Network error loading group data', isError: true);
      GoRouter.of(context).pop();
    }
  ...
  void _populateFormFields() {
    ...
    _groupNameController.text = _groupData!['group_name'] ?? '';
    _groupDescriptionController.text = _groupData!['group_description'] ?? '';
    _groupTagsController.text = _groupData!['group_tags'] ?? '';
    _groupLocationController.text = _groupData!['group_location'] ?? '';

    _groupPrivacy = _groupData!['group_privacy'] ?? 'public';
    _requireApprovalToJoin = _groupData!['require_approval'] ?? false;

    _selectedCategoryId = _groupData!['category_id'];

    // Handle subcategories - they might come as a list of objects or IDs
    if (_groupData!['subcategories'] != null) {
      final subcategories = _groupData!['subcategories'];
      if (subcategories is List) {
        _selectedSubCategoryIds = subcategories
            .map<int>((sub) => sub is Map ? sub['id'] as int : sub as int)
            .toList();
      }
    }

    // Handle images
    _currentGroupImageUrl = _groupData!['group_image'];
    _currentCoverImageUrl = _groupData!['cover_image'];
    ...
    try {
      ...
      if (groupResponse.statusCode == 200 || groupResponse.statusCode == 201) {
        ...
        if (groupData['success'] == true) {
          ...
          String errorMessage =
              groupData['message'] ?? 'Failed to update group';
          ...
          children: [
            Expanded(
              child: _buildImageUploadCard(
                ...
                image: _groupImage,
                currentImageUrl: _currentGroupImageUrl,
            ...
            Expanded(
              child: _buildImageUploadCard(
                ...
                image: _coverImage,
                currentImageUrl: _currentCoverImageUrl,
  ...
  Widget _buildImageUploadCard({
    ...
    // Show new image if selected, otherwise show current image
    Widget imageWidget;
    bool hasImage = image != null || currentImageUrl != null;
