...
class NewLabPrescriptionPage extends StatefulWidget {
  final int? appointmentId;
  final String? patientName;
  final String? patientEmail;
  final String? patientPhone;

  const NewLabPrescriptionPage({
    Key? key,
    this.appointmentId,
    this.patientName,
    this.patientEmail,
  ...
  @override
  State<NewLabPrescriptionPage> createState() => _NewLabPrescriptionPageState();
}

class _NewLabPrescriptionPageState extends State<NewLabPrescriptionPage> {
  ...
  final List<_LabItemControllers> _items = [_LabItemControllers()];
  ...
  void _addItem() => setState(() => _items.add(_LabItemControllers()));
  ...
  Future<void> _submit() async {
    ...
    try {
      final items = _items.map((c) => LabItem(
            testName: c.test.text.trim(),
            specimenType: c.specimen.text.trim().isEmpty ? null : c.specimen.text.trim(),
            urgency: c.urgency,
            notes: c.notes.text.trim().isEmpty ? null : c.notes.text.trim(),
          )).toList();

      final rx = await PrescriptionService.createLab(
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Lab order saved')));
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => LabPrescriptionDetailPage(prescriptionId: rx.id)),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('New Lab Order', style: TextStyle(color: Colors.white)),
        ...
        child: ListView(
          ...
          children: [
              ...
              return Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _text(c.test, 'Test name', required: true),
                    _text(c.specimen, 'Specimen (blood, urine, …)'),
                    Padding(
            ...
            ElevatedButton.icon(
              ...
              label: Text(_submitting ? 'Saving…' : 'Save lab order', style: const TextStyle(color: Colors.white)),
