...
import 'package:xyvra_health/pages/find_doctor/doctor_results_page.dart';

class ModernDoctorFinderSimple extends StatefulWidget {
  const ModernDoctorFinderSimple({Key? key}) : super(key: key);

  @override
  _ModernDoctorFinderSimpleState createState() => _ModernDoctorFinderSimpleState();
}

class _ModernDoctorFinderSimpleState extends State<ModernDoctorFinderSimple> {
  ...
  List<String> _getFilteredSuggestions(String input, List<Map<String, dynamic>> allItems) {
    if (input.isEmpty) return [];

    final inputLower = input.toLowerCase();
    return allItems
        .where((item) => item['name'].toString().toLowerCase().contains(inputLower))
  ...
  Future<void> _searchDoctors() async {
    ...
    try {
      ...
      print('Searching with data: $searchData');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success']) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => DoctorResultsPage(
